...
char __fastcall SetCCreateCameraHook(DWORD thisptr)
{
	if (pLocalPlayer && pDisplay && GetGameState() == GAMESTATE_CHARSELECT)
	{
		...
		pLocalPlayer->Data.X = -0.09f;
		pLocalPlayer->Data.Y = -66.07f;
		pLocalPlayer->Data.Z = 35.13f;
		pLocalPlayer->Data.Heading = 259.25f;
...
void InitHooks()
{
   ...
   if (baseAddress) {
	   ...
	   PatchA((DWORD*)var, "freeporttemple\x00", 15); // Use load instead of "CLZ"
	   ...
	   //Change ZONE NAMES??
	   var = (((DWORD)0x007E222E - 0x400000) + baseAddress); //
	   ...
	   PatchA((DWORD*)byteAddrForPush, "Marr's Last Stand\x00", 18);
